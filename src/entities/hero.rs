use crate::{constants::HERO_HOLY_KNIFE_COOLDOWN, entities::{known_species::SPECIES_HOLY_KNIFE, species::species_by_id}, game_engine::{entity::{Entity, EntityProps}, inventory::{inventory_contains_species, remove_one_of_species_from_inventory}, state_updates::{EngineStateUpdate, WorldStateUpdate}, world::World}, utils::rect::Rect};
...
impl Entity {
    ...
    pub fn update_hero(&mut self, world: &World, time_since_last_update: f32) -> Vec<WorldStateUpdate> {        
        ...
        world_updates.append(&mut self.shoot_holy_knife(world, time_since_last_update));
        world_updates
    }
    ...
    fn shoot_holy_knife(&mut self, world: &World, time_since_last_update: f32) -> Vec<WorldStateUpdate> {
        ...
        if !inventory_contains_species(SPECIES_HOLY_KNIFE) {
            return vec![]
        }

        self.shooting_cooldown_remaining = HERO_HOLY_KNIFE_COOLDOWN;
        remove_one_of_species_from_inventory(SPECIES_HOLY_KNIFE);

        let mut bullet = species_by_id(SPECIES_HOLY_KNIFE).make_entity();
